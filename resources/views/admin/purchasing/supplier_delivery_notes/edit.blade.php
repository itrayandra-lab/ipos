...
@section('title', 'Edit Surat Jalan Supplier')
    ...
    <section class="section">
        <div class="section-header">
            <div class="d-flex align-items-center">
                ...
                <h1>Edit Surat Jalan Supplier</h1>
            </div>
        ...
        <form id="form-sdn" action="{{ route('admin.purchasing.supplier_delivery_notes.update', $sdn->id) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
                    ...
                    <div class="col-12">
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" value="{{ $sdn->sj_number }}" class="form-control" readonly style="background:#f1f5f9; font-family:monospace; font-weight:800;">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>No. SJ Fisik <span class="text-muted">(Opsional)</span></label>
                                            <input type="text" name="delivery_note_number" class="form-control" placeholder="Contoh: SJ/SUP/2026/0012" value="{{ $sdn->delivery_note_number }}">
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <select name="supplier_id" id="supplier_id" class="form-control" required>
                                                ...
                                                @foreach($suppliers as $supplier)
                                                    <option value="{{ $supplier->id }}" {{ $sdn->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="date" name="transaction_date" value="{{ $sdn->transaction_date->format('Y-m-d') }}" class="form-control" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group mb-0">
                                    <label>Catatan</label>
                                    <textarea name="notes" class="form-control" rows="1" placeholder="Catatan tambahan...">{{ $sdn->notes }}</textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mt-3">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                <h4><i class="fas fa-boxes text-teal mr-2"></i> Item Batch</h4>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered mb-0" id="table-items">
                                        ...
                                        <tbody id="tbody-items">
                                            @foreach($sdn->items as $item)
                                            @php
                                                $batch = $item->batch;
                                                $productName = '';
                                                $variantNetto = '';
                                                if ($batch && $batch->product) {
                                                    $productName = ($batch->product->merek->name ?? '') . ' ' . $batch->product->name;
                                                    if ($batch->variant && $batch->variant->netto) {
                                                        $variantNetto = trim($batch->variant->netto->netto_value . ' ' . $batch->variant->netto->satuan);
                                                    }
                                                }
                                            @endphp
                                            <tr data-batch-id="{{ $item->product_batch_id }}">
                                                <td class="text-center font-weight-bold text-muted">{{ $loop->iteration }}</td>
                                                <td>
                                                    <div class="font-weight-bold text-dark" style="font-size:13px;">{{ trim($productName) }}{{ $variantNetto ? ' ' . $variantNetto : '' }}</div>
                                                    <input type="hidden" name="items[{{ $loop->index }}][product_batch_id]" value="{{ $item->product_batch_id }}" class="batch-id-input">
                                                </td>
                                                <td><span class="badge badge-info">{{ $batch->batch_no ?? '-' }}</span></td>
                                                <td><small>{{ $batch->warehouse->name ?? '-' }}</small></td>
                                                <td class="text-center font-weight-bold">{{ $batch ? $batch->current_stock : '-' }}</td>
                                                <td>
                                                    <input type="number" name="items[{{ $loop->index }}][qty]" value="{{ $item->qty }}" min="1" class="form-control qty-input text-center font-weight-bold" required>
                                                </td>
                                                <td><small>{{ $batch && $batch->expiry_date ? $batch->expiry_date->format('d/m/Y') : '-' }}</small></td>
                                                <td class="text-center">
                                                    <button type="button" class="btn btn-sm btn-outline-danger btn-remove-row" data-batch-id="{{ $item->product_batch_id }}" title="Hapus">
                            ...
                            <div class="card-footer bg-whitesmoke text-right">
                                ...
                                <button type="submit" class="btn btn-save">
                                    <i class="fas fa-check mr-2"></i> Perbarui Surat Jalan
                                </button>
                            </div>
                        </div>
                    </div>
...
@push('scripts')
<script>
    let rowCount = {{ $sdn->items->count() }};
    let addedBatchIds = @json($sdn->items->pluck('product_batch_id')->toArray());
    ...
    $(document).ready(function() {
        ...
        $('#form-sdn').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.purchasing.supplier_delivery_notes.update', $sdn->id) }}",
                method: 'POST',
                data: formData,
