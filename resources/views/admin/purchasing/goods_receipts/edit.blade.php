...
<div class="main-content">
    <section class="section">
        ...
        <form id="form-gr" action="{{ route('admin.purchasing.goods_receipts.update', $gr->id) }}" method="POST">
            @csrf
            @method('PUT')
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="purchase_order_id" id="purchase_order_id" class="form-control select2">
                                                ...
                                                @foreach($pos as $po)
                                                    <option value="{{ $po->id }}" {{ $gr->purchase_order_id == $po->id ? 'selected' : '' }}>{{ $po->po_number }} - {{ $po->supplier->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="delivery_note_number" class="form-control" value="{{ $gr->delivery_note_number }}" required>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select name="warehouse_id" id="warehouse_id" class="form-control select2" required>
                                                ...
                                                @foreach($warehouses as $wh)
                                                    <option value="{{ $wh->id }}" {{ $gr->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="supplier_id" id="supplier_id" class="form-control select2" required>
                                                ...
                                                @foreach($suppliers as $supplier)
                                                    <option value="{{ $supplier->id }}" {{ $gr->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            <label>Tanggal Surat Jalan Supplier <span class="text-danger">*</span></label>
                                            <input type="date" name="delivery_date" value="{{ $gr->delivery_date->format('Y-m-d') }}" class="form-control" required>
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label>Tanggal Diterima <span class="text-danger">*</span></label>
                                            <input type="date" name="received_date" value="{{ $gr->received_date->format('Y-m-d') }}" class="form-control" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group mb-0">
                                    <label>Catatan Penerimaan</label>
                                    <textarea name="notes" class="form-control" style="height: 42px;">{{ $gr->notes }}</textarea>
                                </div>
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    @endif
                                                    <input type="text" name="items[{{ $index }}][product_name]" value="{{ $item->product_name }}" class="form-control" {{ $item->purchase_order_item_id ? 'readonly' : '' }}>
                                                </td>
                                                <td>
                                                    <input type="text" name="items[{{ $index }}][description]" value="{{ $item->description }}" class="form-control" {{ $item->purchase_order_item_id ? 'readonly' : '' }}>
                                                </td>
                                                <td>
                                                    <input type="text" name="items[{{ $index }}][satuan]" value="{{ $item->satuan }}" class="form-control text-center" placeholder="pcs/kg...">
                                                </td>
                                                <td>
                                                    <input type="text" class="form-control qty-ordered-input text-center" value="{{ number_format($item->quantity_ordered, 0, ',', '.') }}" readonly>
                                                ...
                                                <td>
                                                    <input type="text" class="form-control qty-diff-display text-center font-weight-bold" value="{{ $item->quantity_difference > 0 ? '+' . number_format($item->quantity_difference, 0, ',', '.') : number_format($item->quantity_difference, 0, ',', '.') }}" readonly>
                                                </td>
                                                <td>
                                                    <input type="text" name="items[{{ $index }}][item_notes]" value="{{ $item->notes }}" class="form-control" placeholder="Catatan...">
                                                </td>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $(document).on('input', '.qty-received-input', function() {
            ...
            let diffText = formatNumberId(diff);
            if (diff > 0) diffText = '+' + diffText;
            diffInput.val(diffText);
        ...
        $('#form-gr').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('admin.purchasing.goods_receipts.update', $gr->id) }}",
                method: "POST",
                data: (function() {
                ...
                res.items.forEach((item, index) => {
                    let html = `
                        <tr>
                            ...
                            <td>
                                <input type="text" class="form-control qty-ordered-input text-center" value="${formatNumberId(item.quantity)}" readonly>
    ...
    function addManualRow() {
        ...
        let html = `
            <tr>
                ...
                <td>
                    <input type="text" class="form-control qty-ordered-input text-center" value="0" readonly>
