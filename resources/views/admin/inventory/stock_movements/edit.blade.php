...
@php
    ...
    foreach ($movement->items as $item) {
        $presets[$item->product_batch_id] = '[' . $item->batch->batch_no . '] ' . ($item->product->merek ? $item->product->merek->name . ' ' : '') . $item->product->name . ' — Stok: ' . $item->batch->qty;
    }
@endphp
<div class="main-content">
    <section class="section">
        <div class="section-header">
            ...
            <h1>Edit Pengiriman Stok {{ $movement->reference_number }}</h1>
        </div>

        <form id="form-movement" action="{{ route('admin.stock_movements.update', $movement->id) }}" method="POST">
            @csrf
            @method('PUT')
                        ...
                        <div class="card">
                            <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select name="from_warehouse_id" id="from_warehouse_id" class="form-control selectric" required>
                                                ...
                                                @foreach($warehouses as $wh)
                                                    <option value="{{ $wh->id }}" {{ $movement->from_warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="to_warehouse_id" id="to_warehouse_id" class="form-control selectric" required>
                                                ...
                                                @foreach($warehouses as $wh)
                                                    <option value="{{ $wh->id }}" {{ $movement->to_warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                ...
                                <div class="form-group mb-0">
                                    <label>Catatan</label>
                                    <textarea name="notes" class="form-control" rows="2" placeholder="Keterangan pengiriman...">{{ $movement->notes }}</textarea>
                                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        @foreach($movement->items as $item)
        addRow({!! $item->product_batch_id !!}, {!! $item->qty !!});
        @endforeach
    ...
    function addRow(batchId, qty) {
        ...
        let html = `
            <tr>
                ...
                <td>
                    <input type="number" name="items[${index}][qty]" class="form-control" min="1" step="1" required placeholder="0" value="${qty || ''}">
                </td>
        ...
        initBatchSelect2($('#table-items tbody tr:last .select-batch'), whId, batchId);
    }

    function initBatchSelect2(el, whId, presetBatchId) {
        el.select2({
        ...
        if (presetBatchId && presets[presetBatchId]) {
            let option = new Option(presets[presetBatchId], presetBatchId, true, true);
