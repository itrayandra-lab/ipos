                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" placeholder="Masukkan Nama" class="form-control" name="name" required="" value="{{ old('name', $user->name) }}">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="email" placeholder="Masukkan Email" class="form-control" name="email" required="" value="{{ old('email', $user->email) }}">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <small class="text-muted">Min 8 Karakter jika ingin diubah</small>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="role" class="form-control selectric" required="">
                                            ...
                                            <option value="admin" {{ $user->role == 'admin' ? 'selected' : '' }}>Admin (Operations)</option>
                                            <option value="sales" {{ $user->role == 'sales' ? 'selected' : '' }}>Sales (Kasir)</option>
                                            <option value="branch" {{ $user->role == 'branch' ? 'selected' : '' }}>Kepala Cabang</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="warehouse_ids[]" class="form-control select2" multiple="" style="width: 100%;">
                                    ...
                                    @foreach($warehouses as $w)
                                        <option value="{{ $w->id }}" {{ in_array($w->id, $userWarehouseIds) ? 'selected' : '' }}>{{ $w->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <div class="permission-group mb-4 p-3 border rounded shadow-sm bg-white">
                                    <div class="d-flex justify-content-between align-items-center bg-light p-2 border-left border-primary mb-3" style="border-left-width: 5px !important;">
                                        ...
                                        @php
                                            ...
                                            $allChecked = $parent && in_array($parent->id, $userPermissionIds);
                                            // Check if all children are checked to correctly set parent status
                                            $children = $items->filter(fn($i) => !str_ends_with($i->slug, '_menu'));
                                            $allChildrenChecked = $children->every(fn($i) => in_array($i->id, $userPermissionIds));
                                        @endphp
                                        @if($parent)
                                            <div class="custom-control custom-checkbox">
                                                <input type="checkbox" name="permission_ids[]" value="{{ $parent->id }}" 
                                                       ...
                                                       {{ $allChecked || $allChildrenChecked ? 'checked' : '' }}>
                                                ...
                                                <div class="col-md-3 mb-2">
                                                    <div class="custom-control custom-checkbox">
                                                        <input type="checkbox" name="permission_ids[]" value="{{ $p->id }}" 
                                                               ...
                                                               {{ in_array($p->id, $userPermissionIds) ? 'checked' : '' }}>
